...
>자바에서는 콜바이 밸루, 콜바이 레퍼런스 로 나타낸다.
>
>말 그래도 값 , 참조에 의한 호출의 문제인데 
>
...
>
>따라서 객체를 보낼때 함수에서 객체를 생성하고 넘겨주어도 주소를 복사한 값이 아닌 다른 객체를 가르키기 때문에 객체 값이 바뀌지 않는다. 대신 객체 안의 필드의 값을 바꾸게 된다면 변경값은 적용이 된다.
...
>
>>for문 안에 보이는 것과 같이 (a,b) in list 로 나누는 것을 객체 디스트럭처링 선언(Destructuring declaration) 이라고 한다. 
>
>
>
>
...
>```
>
>>여기서 주의깊게 봐야 할 점은 Thread.currentThread().name 값이 DefaultDispatcher 라는 것이다. 
>
>### 실행 상태의판단
>
...
Array		arrayOf(), Array(),arrayOfNulls()

arrayOf() -> arrayOf<Int>() || IntArrayOf()

Array() -> Array( size, {i-> i*2}) , IntArray(size, {i ->i+2})

arrayOfNulls() -> arrayOfNulls<Int>(1000)
...
List			listOf			mutableListOf(),arrayListOf(), LinkedList() -> addNodeAtLast(), addNodeAtFront()
